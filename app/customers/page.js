...
export default function Home() {
  ...
  async function fetchCustomers() {
    const data = await fetch(`${APIBASE}/customers`);
    const customers = await data.json();
    setCustomerList(customers);
  }

  // Start edit mode with the selected customer data
  ...
  const deleteById = (id) => async () => {
    ...
    await fetch(`${APIBASE}/customers/${id}`, {
      method: "DELETE",
    });
    fetchCustomers();
  };
  ...
  // Handle customer form submit for adding or updating
          ...
          <ul>
            {customerList.map((customer) => (
              <li key={customer._id}>
                ...
                <Link href={`/customers/${customer._id}`}>{customer.name}</Link> [{customer.memberNumber}]
              </li>
            ))}
          </ul>
        </div>
